/**
 * origin from https://github.com/rafgraph/detect-it/blob/main/src/index.ts
 */
...
export class BrimboriumGestureDetectIt {
  readonly $supportsPassiveEvents=signal(supportsPassiveEvents);
  readonly $supportsPointerEvents=signal(supportsPointerEvents);
  readonly $supportsTouchEvents=signal(supportsTouchEvents);
  readonly $deviceType=signal(deviceType);
  readonly $primaryInput=signal(primaryInput);  
}
