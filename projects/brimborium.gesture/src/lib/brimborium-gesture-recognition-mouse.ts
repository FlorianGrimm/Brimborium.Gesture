import { BrimboriumGestureEvent, BrimboriumGestureSourceEvent, IBrimboriumGestureManager } from "./brimborium-gesture-consts";
        ...
        if ('Down' === this.state) {
            if (gestureSourceEvent.eventType === "mousemove") {
                if (clientPos.distanceTo(this.gestureEventChain.ListPoints[0]) < 10) {
                    ...
                    listOutcome.push({ type: 'effect', effect: this.manager.createDragEffect() });
